...
test('verification flow advances DOM mail filter timestamp after rejected code resend', async () => {
  for (const mailCase of [
    {
      state: { mailProvider: '2925', mail2925Mode: 'provide' },
      mail: { provider: '2925', label: '2925 邮箱' },
    },
    {
      state: { mailProvider: 'icloud' },
      mail: { provider: 'icloud', source: 'icloud-mail', label: 'iCloud 邮箱' },
    },
    {
      state: { mailProvider: 'qq' },
      mail: { provider: 'qq', source: 'qq-mail', label: 'QQ 邮箱' },
    },
  ]) {
    const pollPayloads = [];
    ...
    assert.equal(pollPayloads.length, 2, mailCase.mail.label);
    assert.equal(pollPayloads[0].filterAfterTimestamp, 100000, mailCase.mail.label);
    assert.equal(pollPayloads[1].filterAfterTimestamp, resendRequestedAt, mailCase.mail.label);
    assert.deepEqual(pollPayloads[1].excludeCodes, ['111111'], mailCase.mail.label);
  }
});
